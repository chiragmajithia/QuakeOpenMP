...
#pragma omp parallel private(my_cpu_id) shared(ARCHnodes,bigdist1,bigdist2,temp1,temp2)
  {
#ifdef _OPENMP
...
/* Time integration loop */
            ...
            for (iter = 1; iter <= timesteps; iter++) {
              #pragma omp parallel for private (i,j) firstprivate(ARCHnodes)// private variables in th loop ,defaut are shared
              for (i = 0; i < ARCHnodes; i++)
                for (j = 0; j < 3; j++)
                ...
                #pragma omp parallel for private(i,j) firstprivate(ARCHnodes,disptplus,disptminus)  // runtime 48 sec without firstprivate
                for (i = 0; i < ARCHnodes; i++)                                           // runtume 47 sec with firstprivate arrays
                {                                                                         // runtime 45 secs with ARCHnodes as firstprivate
                  for (j = 0; j < 3; j++)
                  {
...
/* Matrix vector product - basic version                                    */
                ...
                double (*v)[3], double (*w)[3])
              {
                ...
                #pragma omp parallel for private(i,j) collapse(2) shared(numthreads,nodes)
                ...
                #pragma omp parallel private(my_cpu_id) shared(A,Aindex,w1,w2,nodes) //explicit defining shared makes program run faster by 3 seconds numthreads = 16
                {
                    #ifdef _OPENMP
                                      my_cpu_id = omp_get_thread_num();
                    #else
...
#pragma omp parallel for private(i,j) collapse(2) firstprivate(nodes,numthreads) //saves 2 seconds with shared nodes and numthreads
                for (i = 0; i < nodes; i++) {
                  for (j = 0; j < numthreads; j++) {
                    if (w2[j][i]) {
                      w[i][0] += w1[j][i].first;
